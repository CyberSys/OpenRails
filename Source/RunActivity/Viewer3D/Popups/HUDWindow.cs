...
namespace Orts.Viewer3D.Popups
{
    public class HUDWindow : LayeredWindow
    {
        ...
        void TextPageLocomotiveInfo(TableData table)
        {
            ...
            {
                if (train.Cars[i] is MSTSSteamLocomotive || train.Cars[i] is MSTSElectricLocomotive)
                    TableAddLines(table, train.Cars[i].GetDebugStatus());
                else if (train.Cars[i] is MSTSDieselLocomotive)
                {
